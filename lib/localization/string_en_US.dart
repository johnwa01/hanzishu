...
var theString_en_US = [
  ...
  Local(109, "Try a few"),
  Local(110, "The Quanzi Input Method breaks up a Chinese Character into Components and types in the mapped letters on the English keyboard.\n\nFor example, to type character '晶', you break it into components '日', '日', and '日', then type 'B', 'B', and 'B', then choose '晶' from the candidate list below the editing field.\n\nThe rules to break up a character: Follow stroke order (except closing stroke); never break up two lines that are crossing through each other (ok for separated or touching lines); as fewer components as possible; components in earlier positions as big as possible (separated shapes don't have to follow this).\n\nIf having difficulty, you can use [Hint] next to the character.\n\n"),
  ...
  Local(126, "The above chart shows the pairings of the Lead Components of Chinese characters to the English letters in a keyboard. The shapes of two types are very similar. The pairings are the foundation of the Quanzi Input Method！\n\nLet's take a moment to memorize the above Component-key pairings. After that, click 'Start' to test it out.\n\n\Note: Like English, a horizontal line in Chinese can only go from left to right, a vertical line can only go from top to bottom， and two can connect together.\n\nHints: V = split, D = toward left, K = crossing in the middle, H = the horizontal line, Y = the first stroke."),
  Local(127, ""),
  ...
  Local(157, "Components are elements of a Character. The Quanzi Input Method breaks Components into two types， Lead Components and various Expanded Components. Each Lead Component houses a collection of Expanded Components."),
  ...
  Local(177, "DO break up with Components positioned earlier with as many strokes as possible."),
  ...
  Local(183, "Tip 4: The rules to break a Component into subcomponents."),
  ...
  Local(188, "If the Character doesn’t appear on the list, type up to two make-up keys (the first and second subcomponents of the Component) until the Character appears"),
  Local(189, ""),
  Local(190, ""),
  Local(191, "If a Character has two Components:"),
  Local(192, "Type the two Components first"),
  Local(193, "If the Character doesn’t appear on the list, type up to two make-up keys (the first and second subcomponents of the second Component) until the Character appears"),
  Local(194, ""),
  Local(195, ""),
  ...
  Local(336, "Lead/Expanded Component might contain Attached Components. Use the same key of the Component to type the Attached Components.\n\nThe following chart shows that key 'F' has a component '广' owning two Attached Components.\n\nWe'll go through them.\n\n"),
  Local(337, "Lead/Expanded Component might contain Twin Components, which look almost same. Use the same key of the Component to type the Twin Components. \n\nThe following chart shows the components in key 'X' that own some Twin Components.\n\nWe'll go through them.\n\n"),
  ...
  Local(339, "For a character containing only one component, first type the component. Then breakup the component into smaller components (called supplementary components) and type in the first two supplementary components.\n\nThe rules for breaking a Component into supplementary components are same as for breaking a character into components, except you can break lines that are crossing through each other if you have no other options.\n\nFor example, to type a single-component character '吏', first type Component '吏'(O), then break '吏' into supplementary components '一' and '史', then type in '一'(H) and '史'(O). We'll go through some exercises.\n\nDuring breaking up of a component, a few strokes and shapes are not components, and we match them to the closest components as shown below."),
