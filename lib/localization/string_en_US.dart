...
var theString_en_US = [
  ...
  Local(339, "For a character containing only one component, first type the component. Then break up the component into smaller components (called Supplementary Components) and type in the first two Supplementary Components as supplementary keys.\n\nThe rules for breaking up a Component into Supplementary Components are same as for breaking up a character into Components, except if you have no other options you can break lines that are crossing through each other into individual strokes, that is, single stroke components.\n\nExample 1: To type a single-component character '日', first type Component '日'(B), then break up '日' into Supplementary Components '口' and '一', then type in '口'(O) and '一'(H). \n\nExample 2: To type a single-component character '十', first type Component '十'(K), then break up '十' into Supplementary Components '一' and '丨', then type in '一'(H) and '丨'(I). We'll go through some exercises.\n\nDuring breaking up of a Component, a few fold strokes are not Components, and we match them to the closest Components as shown below."),
  Local(340, "For a character containing two Components, first type the two Components. Then type the first two Supplementary Components of the second Component.\n\nFor example, to type '呆', first type in the two Components '口' and '木'. Then break up the second Component into Supplementary Components '十' and '八', then type in '十'(K) and '八'(V) in. \n\nWe'll go through some exercises.\n\n"),
