...
class _TypingSelectionPageState extends State<TypingComponentSelectionPage> {
  ...
  Widget getStartExercise(BuildContext context) {
    return Container(
      child: TextButton(
        child: Text(getString(301)/*"Start"*/, style: TextStyle(fontSize: 28.0 * getSizeRatioWithLimit(), color: Colors.blue),),
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => InputZiPage(typingType: TypingType.ComponentTyping, lessonId: _selectedComponentExerciseNumber!.id, wordsStudy: '', isSoundPrompt: false, inputMethod: InputMethod.Pinxin, showHint: HintType.Hint3, includeSkipSection: false, showSwitchMethod: false),
            ),
          );
        },
      ),
